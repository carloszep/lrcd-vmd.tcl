...
#|    -modified :-2025-11-26.Wed ;;
set lr_trimComplex_version 011
#|  -version :
#|    -011 :
#|      -26nov25 .
#|      -a file _qmreg.qmcharge in now created in apply_restrictions ;
#|    -010 :
#|      -14sep25
#|      -added comments to the 'selTxtR_def' argument .
#|      -parentheses added to the selection text "same residue as ((...)..." .
#|      -argument 'cutoff_noh' added to control H's affecting cutoff ;
#|    -009 :
...
#|      -it is rather a molecule-type category used to select contact residues .
...
#|        -"protein" .
#|        -"protein noh" :
#|          -this omits H atoms in the protein for 'same residue' keyword .
#|          -might be neccesary for MD struct to behave like to x-ray struct ;
#|        -"nucleic" .
#|        -"water" .
#|        -"protein or nucleic" :
...
#|      -default value :-{"not (${selTxtR} or water or ions)"} ;;
#|    -'selTxtR' :
...
#|      -default value :-"" ;;
  ...
  variable selTxtR "protein"
  variable selTxtL [list "not (([set selTxtR]) or water or ions)"]
...
#|      -default value :-"" ;;
#|    -'cutoff_noh', 'ignoreH', 'ignore_H', 'noH' :
#|      -specify hydrogen atoms to be ignored from ligand and receptor select .
#|      -affects the amount of residues selected by interaction cutoff .
#|      -acceptable values :
#|        -"L" :-ligand H atoms ignored ;
#|        -"R" :-receptor H atoms ignored .
#|        -"1", "LR", "all" :-both ligand and receptor H atoms ignored ;
#|        -"0", "none" :-no H atoms ignored ;
#|      -default value :
#|        -"none" ;;
  variable cutoff 4.5
  variable cutoff_qm ""
  variable cutoff_noh "none"
    ...
    if {[expr {[llength $args]%2}] == 0} {
      ...
      foreach {arg val} $args {
        ...
        switch [string tolower $arg] {
          ...
          "cutoff_qm" - "distcutoff_qm" - "distance_qm" - "dist_qm" - "r_qm" \
            {set_cutoff_qm $val}
          "cutoff_noh" - "ignoreh" - "ignore_h" - "noh" {set cutoff_noh $val}
      ...
      "selinfo" {
        if {$selId != ""} {
          ...
          if {([info exists selInfo($selId,ligId)]) && \
            ...
            if {$selTxtL == "not ((${selTxtR}) or water or ions)"} {
...
#|      -create atom selections ;
    proc atomSel {} {
      ...
      variable cutoff; variable cutoff_qm; variable cutoff_noh
...
#          set l_selTxtL_prov $selTxtL
          ...
          switch [string tolower $cutoff_noh] {
            "l" {set nohL "and noh"; set nohR ""}
            "r" {set nohL ""; set nohR "noh"}
            "1" - "lr" - "all" {set nohL "and noh"; set nohR "noh"}
            "0" - "none" {set nohL ""; set nohR ""}
            }
          set selTxtRCad "(same residue as ($selTxtR_def $nohR within $cutoff of ($selTxtLCad $nohL))) and ($selTxtR) and ($resAtm)"
          set selTxtRCad_qm "(same residue as ($selTxtR_def $nohR within $cutoff_qm of ($selTxtLCad_qm $nohL))) and ($selTxtR) and ($resAtm)"
    ...
    foreach restrictType $l_restrictType {
      switch [string tolower $restrictType] {
        "qm/mm" {
          ...
          set harmRSel [atomselect $idC "(backbone or name HA HA1 HA2 HN) or (($selTxtR) and not (resid [$interactCASel get resid] and sidechain)) or ($selTxtLCad_noInt)"]
          ...
          set qmReg [atomselect $idC "(($selTxtR) and resid [$interactCASel_qm get resid] and sidechain) or ($selTxtLCad_qm)"]
          logMsg " selection text used for QM region: [$qmReg text]" $ll2
          set mmReg [atomselect $idC "(($selTxtR) and resid [$interactCASel_qm get resid] and name CA CB) or (protein and resid [$interactCASel_qm get resid] and resname PRO and name N CD)"]
