  ...
  if {$out} {
    foreach cType [list pp pn np nn] {
      ...
      foreach iid $lrcdArr($cType) {
        ...
        puts $loSt "[molinfo $ligId get name].[$lAtS get resname][$lAtS get resid].[$lAtS get name].[$lAtS get index]\t[molinfo $recId get name].[$rAtS get resname][$rAtS get resid].[$rAtS get name].[$rAtS get index]\t$dist"
        $lAtS delete
        $rAtS delete
        }
      }
...
#|  -the list of complexes is read from a text file with three columns
...
#|  -arguments :
#|    -pdbIdsFile :
#|      -name of the text file with PDB Ids, residue names, and chain
#|       _ identifiers .
#|      -format (3 columns) :
#|        -(1) 4-letter PDBID .
#|        -(2) 3-letter PDB residue name for the ligand .
#|        -(3) 1-letter chain identifier (generally A) ;;
#|    -cutoff :
...
proc lrcdVec_writeDB {pdbIdsFile {cutoff 4.5} \
  ...
  if {![file exist $pdbIdsFile]} {
    if {$out} {puts "lrcdVec_writeDB: Error: File not found: $pdbIdsFile"}
    return
    }
  if {$out} {
    puts $loSt "\n+++ lrcdVec_writeDB: Storing LRCD vectors for a set of PDB complexes +++"
  ...
  foreach pdbId $pdbIds {
    ...
    set lrcdMat {}
    if {$pdbId == ""} {break}
    lassign $pdbId pdb lig chain
    if {$out} {
      puts $loSt "Calculating lrcdVec: pdbId: $pdb; lig: $lig; chain: $chain"
      }
# downloading PDB and setting up selInfo array
    set id [mol pdbload $pdb]
    set selInfo(tmpPDB,recId) $id
    set selInfo(tmpPDB,ligId) $id
    set selInfo(tmpPDB,recSelTxt) "protein and chain $chain and noh"
    set selInfo(tmpPDB,ligSelTxt) "resname $lig and chain $chain and noh"
    set selInfo(tmpPDB,title) "pdbId: $pdb; lig: $lig; chain: $chain"
